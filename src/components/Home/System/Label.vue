  ...
  <div class="box">
    ...
    <el-dialog title="创建标签" v-model="showCreate" size="tiny">
      ...
      <div class="list_box" v-show="create.kind == '5'">
        ...
        <div class="list_content">
          <my-unload mId="addLabelImg" :onSuccess="successUnload" :action="action" :onDelete="deleteUnload"
                     :img="create.img"></my-unload>
        </div>
        <div class="clear"></div>
      </div>
    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="普通动态标签" name="普通动态标签">
        ...
        <div class="table_handle">
          <el-button type="primary" @click="deleteLabel(commonDL.checked,'普通动态')" v-if="hasPrivileges('tag_manage')">
            删除
          </el-button>
          ...
          <el-button type="" style="float:right" @click="showCreateNow('3')" v-if="hasPrivileges('tag_add')">新建普通动态标签
          </el-button>
        </div>
        ...
        <div class="table_box">
          <table class="table" id="commonNews">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="commonDL.isIndeterminate" v-model="commonDL.isCheckedAll"
                             @change="checkAllCommonDL($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>普通动态标签</th>
              <th>关联动态标签</th>
            ...
            <tr v-for="(item,index) in commonDL.data">
              ...
              <td>{{item.sort}}</td>
              <td class="operation">
                <a href="javascript:void(0)" @click="showSortNow(item.id,'普通动态')"
                   v-if="hasPrivileges('tag_manage')">排序</a>
                <a href="javascript:void(0)" @click="deleteLabel(item.id,'普通动态')"
                   v-if="hasPrivileges('tag_manage')">删除</a>
              </td>
            </tr>
            </tbody>
          </table>
      ...
      <el-tab-pane label="推荐动态标签" name="推荐动态标签" disabled>
        ...
        <div class="table_box">
          <table class="table">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="recommendDL.isIndeterminate" v-model="recommendDL.isCheckedAll"
                             @change="checkAllRecommendDL($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>推荐动态标签</th>
              <th>简介</th>
      ...
      <el-tab-pane label="技能标签" name="技能标签">
        ...
        <div class="table_handle">
          <el-button type="primary" @click="deleteLabel(skillLabel.checked,'技能标签')" v-if="hasPrivileges('tag_manage')">
            删除
          </el-button>
          <el-button style="visibility:hidden">占位</el-button>
          <el-button type="success" style="float:right" @click="exportExcel('skill')">导出表格</el-button>
          <el-button type="" style="float:right" @click="showCreateNow('2')" v-if="hasPrivileges('tag_add')">新建技能标签
          </el-button>
        </div>
        ...
        <div class="table_box">
          <table class="table" id="skill">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="skillLabel.isIndeterminate" v-model="skillLabel.isCheckedAll"
                             @change="checkAllSkillLabel($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>技能标签</th>
              <th>当前关联人数</th>
      ...
      <el-tab-pane label="兴趣标签" name="兴趣标签">
        ...
        <div class="table_handle">
          <el-button type="primary" @click="deleteLabel(interestLabel.checked,'兴趣标签')"
                     v-if="hasPrivileges('tag_manage')">删除
          </el-button>
          <el-button style="visibility:hidden">占位</el-button>
          <el-button type="success" style="float:right" @click="exportExcel('interest')">导出表格</el-button>
          <el-button type="" style="float:right" @click="showCreateNow('1')" v-if="hasPrivileges('tag_add')">新建兴趣标签
          </el-button>
        </div>
        ...
        <div class="table_box">
          <table class="table" id="interess">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="interestLabel.isIndeterminate" v-model="interestLabel.isCheckedAll"
                             @change="checkAllInterestLabel($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>兴趣标签</th>
              <th>当前关联人数</th>
            ...
            <tr v-for="(item,index) in interestLabel.data">
              ...
              <td>{{item.members}}</td>
              <td class="operation">
                <a href="javascript:void(0)" @click="deleteLabel(item.id,'兴趣标签')"
                   v-if="hasPrivileges('tag_manage')">删除</a>
              </td>
            </tr>
            </tbody>
      ...
      <el-tab-pane label="聚会标签" name="聚会标签">
        ...
        <div class="table_handle">
          <el-button type="primary" @click="deleteLabel(partyLabel.checked,'聚会标签')" v-if="hasPrivileges('tag_manage')">
            删除
          </el-button>
          <el-button style="visibility:hidden">占位</el-button>
          <el-button type="success" style="float:right" @click="exportExcel('party')">导出表格</el-button>
          <el-button type="" style="float:right" @click="showCreateNow('4')" v-if="hasPrivileges('tag_add')">新建聚会标签
          </el-button>
        </div>
        ...
        <div class="table_box">
          <table class="table" id="party">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="partyLabel.isIndeterminate" v-model="partyLabel.isCheckedAll"
                             @change="checkAllPartyLabel($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>聚会标签</th>
              <th>关联聚会数量</th>
            ...
            <tr v-for="(item,index) in partyLabel.data">
              ...
              <td class="operation">
                <a href="javascript:void(0)" @click="showSortNow(item.id,'聚会标签')"
                   v-if="hasPrivileges('tag_manage')">排序</a>
                <a href="javascript:void(0)" @click="showAddSloganNow(item.id)"
                   v-if="hasPrivileges('tag_manage')">添加口号</a>
                <a href="javascript:void(0)" @click="deleteLabel(item.id,'聚会标签')"
                   v-if="hasPrivileges('tag_manage')">删除</a>
              </td>
            </tr>
            </tbody>
      ...
      <el-tab-pane label="服务标签" name="服务标签">
        ...
        <div class="table_handle">
          <el-button type="danger" @click="deleteLabel(serviceLabel.checked,'服务标签')" v-if="hasPrivileges('tag_manage')">
            删除
          </el-button>
          <el-button style="visibility:hidden">占位</el-button>
          <el-button type="success" style="float:right" @click="exportExcel('interest')">导出表格</el-button>
          <el-button type="" style="float:right" @click="showCreateNow('5')" v-if="hasPrivileges('tag_add')">新建服务标签
          </el-button>
        </div>
        ...
        <div class="table_box">
          <table class="table" id="interes">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="serviceLabel.isIndeterminate" v-model="serviceLabel.isCheckedAll"
                             @change="checkAllServiceLabel($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>服务标签</th>
              <th>当前关联人数</th>
            ...
            <tr v-for="(item,index) in serviceLabel.data">
              ...
              <td class="operation">
                ...
                <a href="javascript:void(0)" @click="deleteLabel(item.id,'服务标签')"
                   v-if="hasPrivileges('tag_manage')">删除</a>
          ...
          <el-table :data="children">
            ...
            <el-table-column property="" label="操作">
              <template scope="scope">
                <a href="javascript:void(0)" style="color:#20A0FF"
                   @click="deleteLabel(children[scope.$index].tagId,'服务子标签')" v-if="hasPrivileges('tag_manage')">删除</a>
              </template>
            </el-table-column>
          </el-table>
        </el-dialog>
