  ...
  <div class="box">
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="全部楼咖秀" name="全部楼咖秀">
        <div class="search_box">
          ...
          <div class="searchText">
            <el-select v-model="selectScope.groupId" clearable placeholder="选择商圈" @change="selectArea">
              <el-option
            ...
            <el-select v-model="selectKey.groupId" filterable clearable placeholder="选择社区">
              <el-option
        ...
        <el-card class="dynamics_show" v-show="showDetail">
          ...
          <div class="dynamics_user">
            ...
            <div class="userhead_box">
              <div v-if="detail.author" class="userhead_img"
                   :style="{backgroundImage: 'url('+ detail.author.headPic +')',backgroundSize:'cover',backgroundPosition:'center'}"></div>
            </div>
          </div>
            ...
            <div class="dynamics_imgs">
              ...
              <div class="dynamics_imgBox">
                <div v-if="detail.photo">
                  <div class="show_imgs" v-for="(item,index) in detail.photo"
      ...
      <el-tab-pane label="全部主题" name="全部主题">
        ...
        <div class="table_handle">
          <el-button type="danger" @click="changeThemeState(themes.checked,7)" v-if="hasPrivileges('showtime_manage')">
            删除
          </el-button>
          <el-button type="primary" @click="changeThemeState(themes.checked,0)" v-if="hasPrivileges('showtime_manage')">
            结束
          </el-button>
          <el-button style="visibility:hidden">占位</el-button>
          <el-button type="success" style="float:right" @click="exportExcel('themes')">导出表格</el-button>
          <el-button type="" style="float:right" @click="showCreate" v-if="hasPrivileges('showtime_add')">创建主题
          </el-button>
        </div>
        <delete v-if="themes.showDelete" :checked="themes.checked" :cancel="cancelDeleteThemes"
                :confirm="confirmDeleteThemes"></delete>
        ...
        <el-dialog title="创建主题" v-model="themes.createTheme">
          <div class="block">
            ...
            <div class="issue_list_box">
              ...
              <div class="issue_list_content">
                <my-unload mId="createSL" :onSuccess="successUnloadSL" :action="action" :onDelete="deleteUnloadSL"
                           :img="themes.create.thumbnail"></my-unload>
              </div>
              <div class="clear"></div>
            </div>
            <div class="issue_list_box">
              ...
              <div class="issue_list_content">
                <div class="issuer">
                  <my-unload mId="createZT" :onSuccess="successUnloadZT" :action="action" :onDelete="deleteUnloadZT"
                             :img="themes.create.topicPhoto"></my-unload>
                </div>
              </div>
              <div class="clear"></div>
            </div>
        ...
        <div class="table_box">
          <table class="table" id="themes">
            ...
            <tr>
              <th>
                <el-checkbox :indeterminate="themes.isIndeterminate" v-model="themes.isCheckedAll"
                             @change="checkAllThemes($event.target.checked)"> ID
                </el-checkbox>
              </th>
              <th>主题</th>
              <th>状态</th>
            ...
            <tr v-for="(item,index) in themes.data">
              ...
              <td>{{item.time}}</td>
              <td class="operation">
                <a href="javascript:void(0)" @click="changeThemeState(item.id,7)"
                   v-if="hasPrivileges('showtime_manage')">删除</a>
                <!--                                    <a href="javascript:void(0)">编辑</a>-->
                <a href="javascript:void(0)" @click="changeThemeState(item.id,0)"
                   v-if="hasPrivileges('showtime_manage')">结束</a>
              </td>
            </tr>
            </tbody>
          </table>
  ...
  export default {
    ...
    methods: {
      ...
      selectArea() {
        ...
        this.selectKey.groupId='';
        this.selectScope.groupId&&this.$http.get('http://' + global.URL + '/v1/region/' + this.selectScope.groupId + '/group').then((res) => {
          if (res.body.code == 200 || res.body.code == 201) {
            for (let i = 0; i < res.body.list.length; i++) {
              let o = {
      ...
      resetShowData() {
        ...
        if (this.selectKey.groupId ) {
          parameter += '&groupId=' + this.selectKey.groupId;
        }
